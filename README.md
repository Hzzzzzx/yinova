...
64 卦群控面板 · openclaw 多开 · 一键部署 · v1.0
...
- **阴连不上 / 返回网页而非 JSON**：1) 先点击「阴·启动」；2) 确认 `阴/moltbot.json` 中 `gateway.http.endpoints.chatCompletions.enabled` 为 `true`（install 生成的已包含）；3) 复制项目后请重新执行 `./install.sh` 以修正路径和 token。
...
- **未点击启动却显示某卦已启动**：面板按「端口是否在监听」判断运行状态。若上次关闭终端后网关进程仍在后台，端口会一直被占用。可对显示为「已启动」的卦点「停止」，或点「一键关闭所有终端」释放端口。
